    ...
    <div className="bo-revenue-card" data-reveal>
      <div className="bo-revenue-controls">
        <div className="bo-control-block">
          <div className="bo-control-head"><span>Mon prix mensuel</span><strong>{formatEuro(price)}</strong></div>
          <div className="bo-price-pills" role="group" aria-label="Choisir un prix mensuel">
            {priceOptions.map((value) => (
              <button key={value} type="button" className={price === value ? "is-active" : ""} onClick={() => setPrice(value)}>{value} €</button>
            ))}
          </div>
        </div>
        ...
        <div className="bo-control-block">
          ...
          <input
            ...
            aria-label="Nombre de commerces"
            style={{ "--range-progress": `${clients}%` } as CSSProperties}
          />
      ...
      <div className="bo-revenue-result" aria-live="polite">
        <div><strong>{clients}</strong><span>COMMERCES</span></div>
        <b>×</b>
        <div><strong>{formatEuro(price)}</strong><span>/ MOIS</span></div>
        <b>=</b>
        <div className="bo-revenue-total"><strong>{formatEuro(monthly)}</strong><span>/ MOIS</span></div>
      </div>

      <div className="bo-revenue-foot">
        <strong>Vous choisissez votre prix de vente.</strong>
        <small>Simulation de chiffre d’affaires basée sur le tarif et le nombre de clients sélectionnés.</small>
      </div>
    </div>
  );
}
